...
public class TicketCrudView {
    ...
    public TicketCrudView(MainMenuView mainMenuView) {
    ...
    private void initialize() {
        try {
            ...
            cbSpaceId.setItems(FXCollections.observableArrayList(mainMenuView.getAllParkingSpaces()));
    ...
    private void loadData() {
        masterList.setAll(mainMenuView.getAllTickets());
        updateRecordCount();
    }

    private void updateRecordCount() {
        tfTicketId.setText(String.valueOf(mainMenuView.getParkingTicketController().getNextId()));
    ...
    private void onCreate(ActionEvent actionEvent) {
        ...
        ParkingSpace space = mainMenuView.readParkingSpaceByNumber(spaceNumber, lot);
        if (space == null) {
            CrudAlertHelper.showWarning("Tickets", "Espacio no encontrado");
            return;
        }
        Rate rate = mainMenuView.readRateById(rateId);
        if (rate == null) {
        ...
        try {
            CrudAlertHelper.showResult("Tickets", mainMenuView.createTicket(ticket));
        } catch (IOException e) {
    ...
    private void onRead(ActionEvent actionEvent) {
        ...
        ParkingTicket ticket = mainMenuView.readTicketById(ticketId);
    ...
    private void onUpdate(ActionEvent actionEvent) {
        ...
        try {
            CrudAlertHelper.showResult("Tickets", mainMenuView.updateTicket(ticket));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void onDelete(ActionEvent actionEvent) {
        ...
        try {
            CrudAlertHelper.showResult("Tickets", mainMenuView.deleteTicket(ticket));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private ParkingLot loadLot() {
        ...
        ParkingLot lot = mainMenuView.readParkingLotById(lotId);
        if (lot == null) {
